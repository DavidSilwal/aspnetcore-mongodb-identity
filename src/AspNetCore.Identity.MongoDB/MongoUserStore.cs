...
namespace Dnx.Identity.MongoDB
{
        ...
        where TUser : MongoIdentityUser
    {
        ...
        private readonly MongoUserClaim _claims;
        ...
        {
            var users = await _usersCollection.Find("{}").ToListAsync();
            return users;
        }

        public  Task<string> GetFirstName(TUser user,CancellationToken cancellationToken)
        {
            if (user == null)
            {
               throw new ArgumentNullException(nameof(user));
